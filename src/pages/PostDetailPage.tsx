...
export function PostDetailPage() {
  ...
  return (
    <section className="flex-1 px-4 py-8 max-w-3xl mx-auto">
      <Link to="/" className="inline-flex items-center gap-1 text-stone-600 hover:text-blue-600 mb-6">
        <ArrowLeft size={18} /> Back to Home
      </Link>
      <article className="rounded-xl border border-stone-200 bg-white p-6 shadow-sm">
        ...
        <div className="flex items-center gap-3 mb-4">
          <Avatar src={post.author.photoURL} alt={post.author.name} seed={post.author.id} size={40} />
          <div>
            <p className="font-medium text-stone-800">{post.author.name}</p>
            {dateStr && <p className="text-sm text-stone-500">{dateStr}</p>}
          </div>
        </div>
        <p className="text-stone-600 leading-relaxed whitespace-pre-wrap">{post.description}</p>
      </article>
    </section>
  );
}
